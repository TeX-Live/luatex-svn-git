...
\unexpanded\def\afunction#1%
  {{\ss\bf #1}}

\unexpanded\def\aliteral#1%
  {{\ss "#1"}}

\unexpanded\def\showitemnoret#1#2#3#4#5#6%
  {\hangafter=1\hangindent=12pt
   \noindent {\ss \afunction{#1#6#2}#3}%
   \hskip 12pt plus .5\hsize\penalty0 \hskip 0pt plus -.5\hsize
   #5\par}

\unexpanded\def\showitemret#1#2#3#4#5#6%
  {\hangafter=1\hangindent=12pt
   \noindent {\ss #4 = \afunction{#1#6#2}#3}%
   \hskip 12pt plus .5\hsize\penalty0\hskip 0pt plus -.5\hsize
   #5\par}

\unexpanded\def\luafunction#1#2#3#4#5%
  {\doifemptyelse{#4}
    {\showitemnoret{#1}{#2}{#3}{#4}{#5}{.}}
    {\showitemret{#1}{#2}{#3}{#4}{#5}{.}}\par}

\unexpanded\def\luamethod#1#2#3#4#5%
  {\doifemptyelse{#4}
    {\showitemnoret{#1}{#2}{#3}{#4}{#5}{:}}
    {\showitemret{#1}{#2}{#3}{#4}{#5}{:}}\par}

\unexpanded\def\luacallback#1#2#3#4#5%
  {\doifemptyelse{#4}
...
function showfunction (t,o,f)
     ...
     if method == false then
        context('\\lua%s{%s}{%s}{(%s)}{%s}{%s}', val.type, t, f, table.concat(a,', '),table.concat(r,', '), val.shortdesc)
     else
        context('\\lua%s{%s}{%s}{(%s)}{%s}{%s}', "method", o, f, table.concat(a,', '),table.concat(r,', '), val.shortdesc)
     end
end
...
\NC luatex_hashtype\NC           the hash function for strings used by the current lua interpreter\NC\NR
\NC luatex_hashchars\NC          number of the bits used by the hash function for strings\NC\NR
...
\NC formatname            \NC string  \NC         \NC if no format name was given on the commandline, this will be used \NC \NR

...
\TEX's character code tables (\prm {lccode}, \prm {uccode}, \prm {sfcode}, \prm {catcode}, \prm {mathcode}, \prm {delcode}) can be accessed and written to using
...
The very last string of the very last \type {tex.print()} command in a \prm {directlua} will not have the \prm {endlinechar} appended, all others do.
...
When \LUATEX\ is run with \type {--ini} only the \TEX82 primitives and \prm {directlua} are available, so no extra primitives {\bf at all}.

...
{dimen}), skips (\prm {skip}) and token (\prm {toks}) registers can be accessed
...
influences the string arguments (or numbers turned into strings). Although this
needs to be used with care, you can also pass token or node userdata objects. These
get injected into the stream. Tokens had best be valid tokens, while nodes need to
be around when they get injected.
...
\TB
\NC \type{is_token}     \NC token              \NC checks if the given argument is a token \NC \NR
...
If you really know what you're doing you can create character tokens by not passing a string
but a number:

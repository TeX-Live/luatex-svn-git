...
You can force delimiters to certain sizes by putting an empty box or rule next to
it. The resulting delimiter will either be a character from the stepwise size
range or an extensible. The latter can be quiote differently positioned that the
characters as it depends on the fit aas well as the fact if the used characters
in the font have depth or height. Commands like (plain \TEX s) \type {\big} need
use this feature. In \LUATEX\ we provide a bit more control by supporting optional
parameters \type {height}, \type {depth} and \type {axis}. The following example
uses this:

\startbuffer
\normalleft   height 30pt depth 10pt      \Udelimiter "0 "0 "000028
\quad x\quad
\normalmiddle height 40pt depth 15pt      \Udelimiter "0 "0 "002016
\quad x\quad
\normalright  height 30pt depth 10pt      \Udelimiter "0 "0 "000029
\quad \quad \quad
\normalleft   height 30pt depth 10pt axis \Udelimiter "0 "0 "000028
\quad x\quad
\normalmiddle height 40pt depth 15pt axis \Udelimiter "0 "0 "002016
\quad x\quad
\normalright  height 30pt depth 10pt axis \Udelimiter "0 "0 "000029
\stopbuffer
...
\subsubsection {\type {\mathoption nodelimitershift}}

When set to~1 a vertical delimiter will not be shifted relatiev to the math axis
and its dimensions. This option can be handy when testing macros because this
shifting can confuse testing.

\startbuffer
\dostepwiserecurse{6}{50}{2} {
    \ruledhbox{$\normalleft\Udelimiter "0 "0 "002016
    {\maincolor\vrule height #1pt width 2pt}
    \normalright.$}
}
\stopbuffer
...
\startlinecorrection[blank]
    \startcombination[nx=2,ny=2]
        {\mathoption nodelimitershift 0 \mathematics{\getbuffer}} {\type{0}}
        {\mathoption nodelimitershift 1 \mathematics{\getbuffer}} {\type{1}}
    \stopcombination
\stoplinecorrection

In this example you see the transition from normal stepwise larger symbols to
extensibles. As said, this is only for debugging.

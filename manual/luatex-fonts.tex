...
\NC \bf key                \NC \bf vf \NC \bf tfm \NC \bf used \NC \bf value type \NC \bf description \NC \NR
\NC \tttf name             \NC yes    \NC yes     \NC yes \NC string \NC metric (file) name \NC \NR
\NC \tttf area             \NC no     \NC yes     \NC yes \NC string \NC (directory) location, typically empty \NC \NR
\NC \tttf used             \NC no     \NC yes     \NC yes \NC boolean\NC indicates usage (initial: false) \NC \NR
\NC \tttf characters       \NC yes    \NC yes     \NC yes \NC table  \NC the defined glyphs of this font \NC \NR
\NC \tttf checksum         \NC yes    \NC yes     \NC no  \NC number \NC default: 0 \NC \NR
\NC \tttf designsize       \NC no     \NC yes     \NC yes \NC number \NC expected size (default: 655360 == 10pt) \NC \NR
\NC \tttf direction        \NC no     \NC yes     \NC yes \NC number \NC default: 0 \NC \NR
\NC \tttf encodingbytes    \NC no     \NC no      \NC yes \NC number \NC default: depends on \type {format} \NC \NR
\NC \tttf encodingname     \NC no     \NC no      \NC yes \NC string \NC encoding name \NC \NR
\NC \tttf fonts            \NC yes    \NC no      \NC yes \NC table  \NC locally used fonts \NC \NR
\NC \tttf psname           \NC no     \NC no      \NC yes \NC string \NC This is the \POSTSCRIPT\ fontname in the incoming font
                                                                         source, and it's used as fontname identifier in the \PDF\
                                                                         output. \NC \NR
\NC \tttf fullname         \NC no     \NC no      \NC yes \NC string \NC output font name, used as a fallback in the \PDF\ output
                                                                         if the \type {psname} is not set \NC \NR
\NC \tttf header           \NC yes    \NC no      \NC no  \NC string \NC header comments, if any \NC \NR
\NC \tttf hyphenchar       \NC no     \NC no      \NC yes \NC number \NC default: \TEX's \type {\hyphenchar} \NC \NR
\NC \tttf parameters       \NC no     \NC yes     \NC yes \NC hash   \NC default: 7 parameters, all zero \NC \NR
\NC \tttf size             \NC no     \NC yes     \NC yes \NC number \NC loaded (at) size. (default: same as designsize) \NC \NR
\NC \tttf skewchar         \NC no     \NC no      \NC yes \NC number \NC default: \TEX's \type {\skewchar} \NC \NR
\NC \tttf type             \NC yes    \NC no      \NC yes \NC string \NC basic type of this font \NC \NR
\NC \tttf format           \NC no     \NC no      \NC yes \NC string \NC disk format type \NC \NR
\NC \tttf embedding        \NC no     \NC no      \NC yes \NC string \NC \PDF\ inclusion  \NC \NR
\NC \tttf filename         \NC no     \NC no      \NC yes \NC string \NC the name of the font on disk \NC \NR
\NC \tttf tounicode        \NC no     \NC yes     \NC yes \NC number \NC When this is set to~1 \LUATEX\ assumes per|-|glyph
                                                                         tounicode entries are present in the font. \NC \NR
\NC \tttf stretch          \NC no     \NC no      \NC yes \NC number \NC the \quote {stretch} value from \type
                                                                         {\expandglyphsinfont} \NC \NR
\NC \tttf shrink           \NC no     \NC no      \NC yes \NC number \NC the \quote {shrink} value from \type
                                                                         {\expandglyphsinfont} \NC \NR
\NC \tttf step             \NC no     \NC no      \NC yes \NC number \NC the \quote {step} value from \type
                                                                         {\expandglyphsinfont} \NC \NR
\NC \tttf auto_expand      \NC no     \NC no      \NC yes \NC boolean\NC the \quote {autoexpand} keyword from \crlf
                                                                         \type {\expandglyphsinfont} \NC \NR
\NC \tttf expansion_factor \NC no     \NC no      \NC no  \NC number \NC the actual expansion factor of an expanded font \NC \NR
\NC \tttf attributes       \NC no     \NC no      \NC yes \NC string \NC the \type {\pdffontattr} \NC \NR
\NC \tttf cache            \NC no     \NC no      \NC yes \NC string \NC This key controls caching of the \LUA\ table on the
...
\NC \tttf nomath           \NC no       \NC no    \NC yes \NC boolean\NC This key allows a minor speedup for text fonts. If it
                                                                         is present and true, then \LUATEX\ will not check the
                                                                         character entries for math|-|specific keys. \NC \NR
\NC \tttf slant            \NC no       \NC no    \NC yes \NC number \NC This has the same semantics as the \type {SlantFont}
                                                                         operator in font map files. \NC \NR
\NC \tttf extent           \NC no       \NC no    \NC yes \NC number \NC This has the same semantics as the \type {ExtendFont}
                                                                         operator in font map files. \NC \NR
\stoptabulate
...
\NC \tttf 0    \NC \tttf LT    \NC \tttf  8   \NC \tttf TT    \NC\NR
\NC \tttf 1    \NC \tttf LL    \NC \tttf  9   \NC \tttf TL    \NC\NR
\NC \tttf 2    \NC \tttf LB    \NC \tttf 10   \NC \tttf TB    \NC\NR
\NC \tttf 3    \NC \tttf LR    \NC \tttf 11   \NC \tttf TR    \NC\NR
\NC \tttf 4    \NC \tttf RT    \NC \tttf 12   \NC \tttf BT    \NC\NR
\NC \tttf 5    \NC \tttf RL    \NC \tttf 13   \NC \tttf BL    \NC\NR
\NC \tttf 6    \NC \tttf RB    \NC \tttf 14   \NC \tttf BB    \NC\NR
\NC \tttf 7    \NC \tttf RR    \NC \tttf 15   \NC \tttf BR    \NC\NR
\stoptabulate
...
\starttabulate[|lT|c|]
\NC \bf name      \NC \bf remapping \NC\NR
\NC slant         \NC 1 \NC\NR
\NC space         \NC 2 \NC\NR
...
\NC \bf key          \NC \bf vf \NC \bf tfm \NC \bf used \NC \bf type \NC \bf description \NC\NR
...
\NC \tttf real   \NC this is a base font    \NC\NR
\NC \tttf virtual\NC this is a virtual font \NC\NR
\stoptabulate
...
\starttabulate[|l|p|]
\NC \bf value      \NC \bf description                                           \NC \NR
\NC \tttf type1    \NC this is a \POSTSCRIPT\ \TYPEONE\ font                     \NC \NR
\NC \tttf type3    \NC this is a bitmapped (\PK) font                            \NC \NR
\NC \tttf truetype \NC this is a \TRUETYPE\ or \TRUETYPE|-|based \OPENTYPE\ font \NC \NR
\NC \tttf opentype \NC this is a \POSTSCRIPT|-|based \OPENTYPE\ font             \NC \NR
\stoptabulate
...
\starttabulate[|l|p|]
\NC \bf value    \NC \bf description \NC \NR
\NC \tttf no     \NC don't embed the font at all \NC \NR
\NC \tttf subset \NC include and atttempt to subset the font \NC \NR
\NC \tttf full   \NC include this font in its entirety \NC \NR
\stoptabulate
...
\NC \bf command name  \NC \bf arguments \NC \bf type  \NC \bf description \NC\NR
\NC \tttf font        \NC 1             \NC number    \NC select a new font from the local \type {fonts} table\NC\NR
\NC \tttf char        \NC 1             \NC number    \NC typeset this character number from the current font,
                                                          and move right by the character's width\NC\NR
\NC \tttf node        \NC 1             \NC node      \NC output this node (list), and move right
                                                          by the width of this list\NC\NR
\NC \tttf slot        \NC 2             \NC number    \NC a shortcut for the combination of a font and char command\NC\NR
\NC \tttf push        \NC 0             \NC           \NC save current position\NC\NR
\NC \tttf nop         \NC 0             \NC           \NC do nothing \NC\NR
\NC \tttf pop         \NC 0             \NC           \NC pop position \NC\NR
\NC \tttf rule        \NC 2             \NC 2 numbers \NC output a rule $ht*wd$, and move right.\NC\NR
\NC \tttf down        \NC 1             \NC number    \NC move down on the page\NC\NR
\NC \tttf right       \NC 1             \NC number    \NC move right on the page\NC\NR
\NC \tttf special     \NC 1             \NC string    \NC output a \type {\special} command\NC\NR
\NC \tttf lua         \NC 1             \NC string    \NC execute a \LUA\ script (at \type {\latelua} time)\NC\NR
\NC \tttf image       \NC 1             \NC image     \NC output an image (the argument can be either an \type
                                                          {<image>} variable or an \type {image_spec} table)\NC\NR
\NC \tttf comment     \NC any           \NC any       \NC the arguments of this command are ignored\NC\NR
\stoptabulate

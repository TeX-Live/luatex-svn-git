...
\TB
\NC \lpr {luatexbanner}   \NC the banner reported on the command line   \NC \luatexbanner      \NC \NR
\NC \lpr {luatexversion}  \NC a combination of major and minor number   \NC \the\luatexversion \NC \NR
\NC \lpr {luatexrevision} \NC the revision number, the current value is \NC \luatexrevision    \NC \NR
\LL
\stoptabulate
...
to accommodate for a larger range of acceptable numbers. For instance, \prm {char} now accepts values between~0 and $1{,}114{,}111$. This should not be a
...
or secondary (right of the equals sign) value are: \prm {char}, \prm {lccode}, \prm {uccode}, \lpr {hjcode}, \prm {catcode}, \prm {sfcode},
\lpr {efcode}, \lpr {lpcode}, \lpr {rpcode}, \prm {chardef}.
...
Boxes (like \prm {hbox} and \prm {vbox}) become \type {hlist} or \type
...
displayed using \type {\the\toks}. On the \LUA\ side, each \prm {directlua}
...
have some impact. For this reason there is a variant call available: \lpr {luafunction}. This command is used as follows:
...
The \lpr {luafunctioncall} primitive does the same but is unexpandable, for instance
in an \prm {edef}.
...
assignment, making the requested csname for the font \prm {ifx} equal to \prm {nullfont}, so that it can be tested against that without bothering the user.
...
The \prm {vsplit} primitive has to be followed by a specification of the

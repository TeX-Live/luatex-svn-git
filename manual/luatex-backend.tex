...
\subsection{\type {[set|get][major|minor]version}}
...
\subsection{\type {[set|get]inclusionerrorlevel} and \type {[set|get]ignoreunknownimages}}
...
\subsection{\type {[set|get]suppressoptionalinfo}, \type {[set|get]trailerid}
...
This optional info bitset (a number) determines what kind of info gets flushed.
By default we flush all.

You can set your own trailer id. This has to be string containing valid \PDF\
array content with checksums.

This cidset flag (a number) disables inclusion of a so called CIDSet which can be
handy when aiming at some of the many \PDF\ substandards.

\subsection{\type {[set|get][obj|]compresslevel} and \type {[set|get]recompress}}
...
\subsection{\type {getmaxobjnum} and \type {getobjtype}, \type {getfontname},
...
\subsection {\type {get[string|integer|number|boolean|name]}}
...
\subsection {\type {[open|close|readfrom][whole|]stream}}
...
\subsection {\type {getfrom[dictionary|array]}}

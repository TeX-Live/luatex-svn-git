...
When you're in a scriptrun the handler assigned to \type {run_script} can fetch
variables from \METAPOST\ by name, given that they are accessible.

\starttyping
<number>  = get_numeric("foo")
<boolean> = get_boolean("foo")
<string>  = get_string ("foo")
<table>   = get_path   ("foo")
\stoptyping

A path table has entries that themselves are tables of six numbers. The table
also has a \type {cycle} key indicating if we're dealing with a cycle.

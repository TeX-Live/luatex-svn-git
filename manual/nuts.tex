...
on that property but treat is an abstraction just like traditional nodes. In fact,
...
index by key variant was also optimized, so direct access only make sense when
...
have been optimized and the most frequently accessed nodes and fields hav ea higher
priority.

Because in practice the \type {next} accessor result sin a function call, there
...
node and fields types never get seen, or seen only a few times. Most function in the
\type {node} namespace have a compation in \type {node.direct}, but of course not the

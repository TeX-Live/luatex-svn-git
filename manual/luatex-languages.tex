...
were current when the node was created: the \prm {setlanguage} (15 bits), \prm {lefthyphenmin} (8 bits), \prm {righthyphenmin} (8 bits), and \prm {uchyph}
(1 bit).
...
\startitem
    ...
    discretionary after sensing an input character that matches the \prm {hyphenchar} in the current font. This test is wrong in our opinion: whether
    ...
    characters that matches the value of the new integer parameter \prm {exhyphenchar}, it will insert an explicit discretionary after that series
...
\startitem
    ...
    \prm {setlanguage} is changed so that it is now an integer parameter like
    all others. That integer parameter is used in \type {\glyph_node} creation to
    add language information to the glyph nodes. In conjunction, the \prm {language} primitive is extended so that it always also updates the value of
    \prm {setlanguage}.
\stopitem
...
non|-|\UNICODE\ characters. Likewise, the expanded argument for \prm {hyphenation} also has to be proper \UTF8, but here a bit of extra syntax is
provided:
...
have \type {hjcode}, the case relate codes can be used exclusively for \prm {uppercase} and \prm {lowercase}.
...
\stopitem
\startitem
    \LUATEX\ uses the language-specific variables \lpr {prehyphenchar} and \lpr {posthyphenchar} in the creation of implicit discretionaries, instead of
    \TEX82's \prm {hyphenchar}, and the values of the language|-|specific variables
    \lpr {preexhyphenchar} and \lpr {postexhyphenchar} for explicit

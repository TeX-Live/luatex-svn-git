...
argument, as there typically will be only one `line', so that comment
...
the `normal' scanning of \type{\relax}).
\stopitemize
...
\subsection{Applying a `feature file'}
...
\subsection{Applying an `afm file'}
...
This library will provide the interface to \LUATEX's structure
...
records, so that the resulting `horizontal list' contains the final
...
the hyphenated result back into a `horizontal list' that is then
spliced back into the main paragraph stream.

The \type{char_node} records are somewhat misnamed, as they are glyph
posisitions in specific fonts, and therefore not really `characters'
...
identifiers so that the records are `glyph nodes' from now on.
...
\TEX82 differentiated between \type{char_node}s and \type{lig_node}s. 
The former were simple items that contained nothing but a `character'
and a `font' field, and they lived in the same memory are as
tokens. The latter also contained a list of components, and a subtype
indicating whether this ligature was the result of a word boundary,
and it was stored in the same place as other nodes like boxes and
kerns and glues.
...
`attr' (see~\in{section}[glyphnodes]), these nodes also contain:

\startitemize
\item A subtype, split into four main types: 
   \startitemize 
   \item `character' -- for characters to be hyphenated
   \item `glyph'  -- for specific font glyphs
   \item `ligature'  -- for ligatures
   \item `ghost'  -- for `ghost objects', explained  below
   \stopitemize
   The latter two make further use of two extra fields:
   \startitemize 
   \item `left'  -- for ligatures: created from a left word boundary. 
                    for ghosts:  created from \tex{leftghost}
   \item `right' -- for ligatures: created from a right word boundary. 
                    for ghosts:  created from \tex{rightghost}
   \stopitemize
   for ligatures, both bits can be set at the same time (in case of a single-glyph word).

\item \type{glyph_node}s of type `character' also contain language data, 
...
the box. In \LUATEX, all language variables are already fixed.
...
`character', but there are a few small exceptions.

First, the \tex{accent} primitives creates nodes with subtype `glyph'
...
`character' nodes, it is possible to achieve the same effect.

This change of meaning did happen with \tex{char}, that now generates
`character' nodes, consistent with it's changed meaning in \XETEX.
...
nodes with `glyph' subtypes.

Third, the \ALELPH-derived commands \tex{leftghost} and
\tex{rightghost} create nodes of a third subtype: `ghost'. These nodes
...
primitive will likely be created to indicate the `explicit hyphenation
...
Sixth, the \tex{noboundary} command now does create whatsits. These
whatsits are needed because the exact place of the \tex{noboundary}
command in the input stream has to be retained until after the
ligature and font processing stages.
...
that processing code outside of the `main control' loop, and only the
first character of any `word' was handled by that `main control' loop.
...
\section{Loading patterns and exceptions}
...
\NC ta-ble               \NC table            \NC \type{ta\-ble}  
   ($=$ \type{ta\discretionary{-}{}{}ble})\NC\NR
\NC vis=à=vis            \NC vis-à-vis        \NC \type{vis-à-vis}\NC\NR % fix utf
\NC user=\discretionary{}{}{}friendly  \NC user-friendly    \NC \type{user-\discretionary{}{}{}friendly}\NC\NR
\NC user=-friendly       \NC user-friendly    \NC \type{user-\-friendly} 
   ($=$ \type{user\discretionary{--}{}{-}friendly}) \NC\NR
\NC ba{k-}{k}{c}ken  \NC backen \NC \type{ba\discretionary{k-}{k}{c}ken}\NC\NR
\stoptabulate

Keep in mind that, as explained above, \LUATEX\ does not currently
insert automatic discretionaries, so the word `vis-à-vis' will never
...
First and foremost, there is no `compressed trie' involved (\LUATEX\
uses a finite state hash, like OpenOffice). Differences between
...
\item Because there is no `trie preparation' stage, language patterns 
  never become frozen. This means that the primitive \tex{patterns}
  (and it's lua counterpart \texlua{lang.patterns}) can be used at any
...
hyphenation code), they are editted in place.
...
aware that this function expects to receive a list of `character'
nodes. It will not operate properly in the presence of `glyph',
`ligature', or 'ghost' nodes, nor does it know how to deal with
kerning.  In the near future, it will be able to skip over `ghost'
...
\LUATEX\ will process the list to convert the `character' nodes into 
`glyph' and `ligature' nodes. This is actually done in two stages:
...
the ligaturing stage adds virtual `character' nodes to the word
...
items after it is done with them, and it does the same for `ghost'
nodes. Finally, at the end of the kerning stage, all remaining
`character' nodes are converted to `glyph' nodes.
...
using a `normal' font with all the \type{f}-\type{i} ligatures:
\starttabulate[|l|l|]
...
assertion error or ``Emergency stop''.

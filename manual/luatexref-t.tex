...
for the stable version. Are current estimate expects that to become
...
\item Parts of the Font Parser from \FONTFORGE\ 2007.06.07
...
Note that the \type{\luatexdatestamp} depends on both the compilation time and place of the current
executable, it is defined in terms of the local time. The purpose of this primitive is solely to be
...
For now, it only makes sense to use values above the base plane (\type{"0xFFFF})
for \tex{mathcode} and \tex{catcode} assignments, since the
...
\NC position     \NC table    \NC \type {vr} table\NC\NR
\NC pair         \NC table    \NC one string: \type {paired}, and a \type {vr} (sub)table\NC\NR
...
almost completely (only TRT and TLT are actually tested).
...
\item Add line numbers to the lua errors
\stopitemize

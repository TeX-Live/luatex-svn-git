...
Furthermore, it disables loading of compiled \LUA\ libraries
(support for these was added in 0.46.0).
...
version is 1.1.5.  See \hyphenatedurl{http://luajit.org/coco.html} for details. This
...
Note: the above functions and tables are likely to go away in a future
version because one can just as well use the direct interface to the 
box node via \type{tex.box} as documented below.
...
interface to \TEX's internal, 100\% portable scale function will be
...
\subsubsubject{\luatex{pdf.pdfmapline}, \luatex{pdf.pdfmapfile} (new in 0.47.0)}

These two variables can be used to replace \type{\pdfmapline} and
\type{\pdfmapfile} from \PDFTEX. The values are write-only: when
queried they return \type{nil}.

Note: this interface will almost certainly change in the future.

\subsubsubject{\luatex{pdf.pdfcatalog}, \luatex{pdf.pdfinfo},
    \luatex{pdf.pdfnames}, \luatex{pdf.pdftrailer}  (new in 0.47.0)}

These variable offer a read-write interface to the corresponding

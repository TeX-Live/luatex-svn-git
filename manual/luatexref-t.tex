...
allocations is \type {"10FFFF} or $2^{20}+2^{16}-1$ (21 bits). The
...
\NC \tex{char}    \NC 21       \NC \tt 10FFFF \NC $2^{20}+2^{16}-1$ \NC\NR
\NC \tex{chardef} \NC 21       \NC \tt 10FFFF \NC $2^{20}+2^{16}-1$ \NC\NR
\NC \tex{lccode}  \NC 21       \NC \tt 10FFFF \NC $2^{20}+2^{16}-1$ \NC\NR
\NC \tex{uccode}  \NC 21       \NC \tt 10FFFF \NC $2^{20}+2^{16}-1$ \NC\NR
\NC \tex{sfcode}  \NC 15       \NC \tt ~~7FFF \NC $2^{15}-1$        \NC\NR
\NC \tex{catcode} \NC ~4       \NC \tt ~~~~~F \NC $2^{4}-1$         \NC\NR
\stoptabulate
...
While it is nice to have these math parameters available for tweaking, it would be tedious to have to set
each of them by hand. For this reason, \LUATEX\ initializes a bunch of these parameters whenever you assign a
font identifier to a math family based on either the traditional math font dimensions in the font (for
assignemnts to math family 2 and 3 using TFM-based fonts like \type{cmsy} and \type{cmex}), or based on the
named values in a potential \type{MathConstants} table when the font is loaded via Lua.  If there is a
\type{MathConstants} table, this takes precendence over font dimensions, and no attention is payed to
which family is being assigned to.

In the table below, the one-letter style abbreviations and symbolic tfm font dimension names match those
using in the \TeX book. Assignments to \type{\textfont} set the values for the cramped and uncramped display
and text styles. \type{\scriptfont} for the script styles, and \type{\scriptscriptfont} for the scriptscript
styles. In the tfm case, assignments only happen in family 2 and family 3 (and of course only for the
parameters for which there are font dimensions).
...
\NC \tex{Umathradicaldegreeraise}\NC --                   \NC RadicalDegreeBottomRaisePercent          \NC <not set>$^2$ \NC\NR
...
\NC \tex{Umathsubsupshiftdown}  \NC --                    \NC SubscriptShiftDown                       \NC sub2 \NC\NR
...
\NC \tex{Umathunderbarkern}     \NC --                    \NC UnderbarExtraDescender                   \NC 0$^5$ \NC\NR
...
This provides a partial backward compatibility with \TEX82, but that compatibilty is only partial:
...
Note 5: Tfm fonts set \tex{Umathunderbarkern} and \tex{Umathconnectoroverlapmin} to zero because
\TeX82\ never adds a kern below the underbar and always stacks extensibles without any overlap.
...
\subsection{Math spacing setting}

Besides the parameters mentioned in the previous sections, there are
also 64 primitives to control the math spacing table (as explained in
...
Note 1: for ease of use as well as for backward compatibity, \type{\thinmuskip},
...
\subsection{Math accent handling}

If a math accent has to be placed and the accentee is a character and has a non-zero
...
\subsection{Math root extension}
...
The values of \type{top_accent} is used only for math accent placement for
OpenType-based math fonts, see the Math changes section earlier in this manual for
details.

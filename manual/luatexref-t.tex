...
However, starting with beta \type{0.40.0}, most of that functionality
...
More fine-grained control is possible, you can look the details in
...
plane (\type{"FFFF}) for \tex{mathcode} and \tex{catcode}
...
instance, \tex{char} now accepts values between~0 and $1{,}114{,}111$. This
...
\NC \tex{catcode} \NC \hphantom{2}4 \NC \tt ~~~~~F \NC $2^{4}-1$         \NC\NR
\stoptabulate
...
This library used to be called \quote{fontforge}. The library
is still available under that name for now,
...
looking at the supplied extension.
...
The following items are all still todo (most of them hopefully before \LUATEX\ 0.40
comes out).

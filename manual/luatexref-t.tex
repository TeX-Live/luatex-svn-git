...
\LUAJITTEX\ has also: 
\startitemize[packed]
\item the  \type{--jiton}  options that turns the JIT compiler on (default off) and 
\item the \type{--jithash=STRING}   that selects the hash function for the lua strings (\type{lua51|luajit20}: default \type{lua51}.
\stopitemize

Still for the hash, \LUATEX\ has the \type{--luahashchars} option
that shows  the bits used by current Lua interpreter for strings hashing --- more on hashing at the end od the section.

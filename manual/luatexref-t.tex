...
functionality currently (0.42) does not work on non-intel OpenBSD platforms nor on
powerpc Linux-es.
...
\subsubsection{\luatex{node.dimensions}}
...
The current \type{h} and \type{v} values
that define the location on the output page.
The values can be queried and set using scaled points as units.
...
The function calls associated with \type{pdf.h} and \type{pdf.v} are
...
\NC psname             \NC no       \NC no       \NC yes \NC string \NC actual input (\POSTSCRIPT) name (this is the PS fontname in the incoming font source)\NC\NR

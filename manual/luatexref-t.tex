...
is based on the MPlib beta version 0.20 (based on \MP\ 1.002).
...
\NC mode       \NC   the file mode: 'r', 'rb', 'w', or 'wb' \NC \NR
...
The chunk has to obey proper nesting as if it was a file (so
begingroup and endgroups have to be balanced).
...
with a few possible keys (only `state' is always guaranteed to be present).

\starttabulate[|l|l|p|]
\NC log    \NC string \NC output to the 'log' stream \NC \NR
\NC term   \NC string \NC output to the 'term' stream \NC \NR
\NC error  \NC string \NC output to the 'error' stream \NC \NR
\NC state  \NC number \NC the return value. 0 = good, 1 = warning, 2 = errors, 
                         3 = fatal error \NC \NR
\NC fig    \NC table \NC An array of generated figures (if any)\NC \NR
\stoptabulate

When \type{state} equals 3, you should stop using this MPlib instance
...
There is an \type{obj:fields()} command to get the list of accessible
values for a particular object (but you can just as easily use the
tables given below). 
...
\NC dsize \NC number \NC font design size (in big points times 65536)\NC \NR
\NC color \NC table \NC the object's color \NC \NR
...
\subsubsection{Knot lists}

Paths and pens (that are really just a special type of paths as far as
MPlib is concerned) are represented

\starttabulate[|l|l|p|]
\NC originator	\NC string \NC One of 'program', 'user'\NC \NR
\NC left_type	\NC string \NC One of 'endpoint', 'explicit'
                               (normal knots are 'explicit')\NC \NR
...
A color is a hash table with keys:

\starttabulate[|l|l|p|]
\NC model \NC string \NC one of 'none', 'grey', 'rgb', 'cmyk', 'uninitialized' \NC\NR
\NC rgb   \NC table  \NC A 3-item array with values in the range 0 .. 1 \NC\NR
\NC cmyk  \NC table  \NC A 4-item array with values in the range 0 .. 1 \NC\NR
\NC grey  \NC number \NC A number with value in the range 0 .. 1 \NC\NR
\stoptabulate

If the color model is \type{unitialized}, then all three value sets are present,
otherwise only the applicable one (none at all for color model \type{none}).
...
\NC 4 \NC number \NC represents xy \NC\NR
\NC 5 \NC number \NC represents yx \NC\NR
\NC 6 \NC number \NC represents yy \NC\NR
\stoptabulate

\subsubsection{Dashes}

Each dashlist is two-item hash.

...
\item a still experimental version of the \METAPOST\ library
...
\NC \tex{luatexrevision}  \NC the revision, as in \PDFTEX;
...
Note: for now, it only makes sense to use values above the base
plane (\type{"0xFFFF}) for \tex{mathcode} and \tex{catcode}
assignments, since the hyphenation patterns are still limited to
max.\ 16-bit values, so the other commands will not know what to
do with those high values.

A few primitives are affected by this, all in a similar fashion: each of
them has to accommodate for a larger range of acceptable numbers.  For
...
Text handling is now extended up to the full \UNICODE\ range. The extension
from 8-bit to 16-bit was already present in \ALEPH\ by means of a set of
extra primitives starting with the \type{\o} prefix, the extension to full
Unicode (the \type{\U} prefix) is compatible with \XETEX.

The math primitives from \TEX\ and \ALEPH\ are kept mostly
as they are, except for the ones that convert from input to math
commands like \type{mathcode} and \type{omathcode} that now allow
for a larger argument on the left hand side of the equals sign.

% hahe: again wrong by 1?
\starttabulate[|l|l|l|l|]
\NC \bf primitive     \NC \bf max index/bits \NC \bf value range (in hex)  \NC\NR
\NC \tex{mathchardef} \NC 15             \NC \tt~~~8000                    \NC\NR
\NC \tex{mathcode}    \NC 8=15           \NC \tt~~~~~FF = ~~~800           \NC\NR
\NC \tex{delcode}     \NC 8=24           \NC \tt~~~~~FF = ~FFFFF           \NC\NR
\NC \tex{mathchar}    \NC 15             \NC \tt~~~7FFF                    \NC\NR
\NC \tex{mathaccent}  \NC 15             \NC \tt~~~7FFF                    \NC\NR
\NC \tex{delimiter}   \NC 27             \NC \tt7FFFFFF                    \NC\NR
\NC \tex{radical}     \NC 27             \NC \tt7FFFFFF                    \NC\NR
\NC \tex{omathchardef}\NC 27             \NC \tt8000000                    \NC\NR
\NC \tex{omathcode}   \NC 21=27          \NC \tt~10FFFF = 8000000          \NC\NR
\NC \tex{odelcode}    \NC 21=24+24       \NC \tt~10FFFF = ~FFFFFF + ~FFFFFF\NC\NR
\NC \tex{omathchar}   \NC 27             \NC \tt7FFFFFF                    \NC\NR
\NC \tex{omathaccent} \NC 27             \NC \tt7FFFFFF                    \NC\NR
\NC \tex{odelimiter}  \NC 27+24          \NC \tt7FFFFFF + ~FFFFFF          \NC\NR
\NC \tex{oradical}    \NC 27+24          \NC \tt7FFFFFF + ~FFFFFF          \NC\NR
\NC \tex{Umathchardef}\NC 3+8+21         \NC \tt8+FF+10FFFF                \NC\NR
\NC \tex{Umathcode}   \NC 21=3+8+21      \NC \tt~10FFFF = 8+FF+10FFFF      \NC\NR
\NC \tex{Udelcode}    \NC 21=8+21        \NC \tt~10FFFF = FF+10FFFF        \NC\NR
\NC \tex{Umathchar}   \NC 3+8+21         \NC \tt8+FF+10FFFF                \NC\NR
\NC \tex{Umathaccent} \NC 3+8+21         \NC \tt8+FF+10FFFF                \NC\NR
\NC \tex{Udelimiter}  \NC 3+8+21         \NC \tt8+FF+10FFFF                \NC\NR
\NC \tex{Uradical}    \NC 8+21           \NC \tt  FF+10FFFF                \NC\NR
\NC \tex{Umathcharnum}\NC 32             \NC \tt see below                 \NC\NR
\NC \tex{Umathcodenum}\NC 21=32          \NC \tt~10FFFF = see below        \NC\NR
\NC \tex{Udelcodenum} \NC 21=29          \NC \tt~10FFFF = see below        \NC\NR
\stoptabulate

Note: for \type{\Umathcharnum} and \type{\Umathcodenum}, all information is
packed into a single signed integer: the lowest 21 bits are the character
code, the 3 bit above that represent the math class, and the family data is
kept in the topmost bits (This means that the values for math families
128--255 are actually negative).  For \type{\Udelcodenum} there is no math
class; the math family information is stored in the bits directly on top of
the character code. Using these three commands is not as natural as using
...
\section{Additional math commands}

\subsection{Math style setting}
...
These additonal commands are not all that valuable on their own, but
they come in handy as arguments to the math parameter settings that
will be added shortly.

\subsection{Math parameter setting}

\starttabulate
\NC primitive name                  \NC description \NC \NR
...
\NC \type{\Umathfractiondenomdown}  \NC Denominator shift downwar in \type{\over} \NC \NR
\NC \type{\Umathfractiondelsize}    \NC \NC \NR
\NC \type{\Umathlimitabovevgap}     \NC \NC \NR
\NC \type{\Umathlimitabovebgap}     \NC \NC \NR
\NC \type{\Umathlimitabovekern}     \NC \NC \NR
\NC \type{\Umathlimitbelowvgap}     \NC \NC \NR
\NC \type{\Umathlimitbelowbgap}     \NC \NC \NR
\NC \type{\Umathlimitbelowkern}     \NC \NC \NR
\NC \type{\Umathsubshiftdrop}       \NC \NC \NR
\NC \type{\Umathsupshiftdrop}       \NC \NC \NR
\NC \type{\Umathsubshiftdown}       \NC \NC \NR
\NC \type{\Umathsubsupshiftdown}    \NC \NC \NR
\NC \type{\Umathsubtopmax}          \NC \NC \NR
\NC \type{\Umathsupshiftup}         \NC \NC \NR
\NC \type{\Umathsupbottommin}       \NC \NC \NR
\NC \type{\Umathsupsubbottommax}    \NC \NC \NR
\NC \type{\Umathsubsupvgap}         \NC \NC \NR
\NC \type{\Umathspaceafterscript}   \NC \NC \NR
\stoptabulate

Each of these is set by a command like this:
...
\NC axis                \NC --                    \NC AxisHeight                               \NC axis_height \NC\NR
\NC fraction_del_size   \NC D, D'                 \NC 0                                        \NC delim1 \NC\NR
\NC "                   \NC T, T', S, S', SS, SS' \NC 0                                        \NC delim2 \NC\NR
\NC fraction_denom_down \NC D, D'                 \NC FractionDenominatorDisplayStyleShiftDown \NC denom1 \NC\NR
\NC "                   \NC T, T', S, S', SS, SS' \NC FractionDenominatorShiftDown             \NC denom2 \NC\NR
\NC fraction_denom_vgap \NC D, D'                 \NC FractionDenominatorDisplayStyleGapMin    \NC 3*default_rule_thickness \NC\NR
\NC "                   \NC T, T', S, S', SS, SS' \NC FractionDenominatorGapMin                \NC default_rule_thickness \NC\NR
\NC fraction_num_up     \NC D, D'                 \NC FractionNumeratorDisplayStyleShiftUp     \NC num1 \NC\NR
\NC "                   \NC T, T', S, S', SS, SS' \NC FractionNumeratorShiftUp                 \NC num2 \NC\NR
\NC fraction_num_vgap   \NC D, D'                 \NC FractionNumeratorDisplayStyleGapMin      \NC 3*default_rule_thickness \NC\NR
\NC "                   \NC T, T', S, S', SS, SS' \NC FractionNumeratorGapMin                  \NC default_rule_thickness \NC\NR
\NC fraction_rule       \NC --                    \NC FractionRuleThickness                    \NC default_rule_thickness \NC\NR
\NC limit_above_bgap    \NC --                    \NC UpperLimitBaselineRiseMin                \NC big_op_spacing3 \NC\NR
\NC limit_above_kern    \NC --                    \NC 0                                        \NC big_op_spacing5 \NC\NR
\NC limit_above_vgap    \NC --                    \NC UpperLimitGapMin                         \NC big_op_spacing1 \NC\NR
\NC limit_below_bgap    \NC --                    \NC LowerLimitBaselineDropMin                \NC big_op_spacing4 \NC\NR
\NC limit_below_kern    \NC --                    \NC 0                                        \NC big_op_spacing5 \NC\NR
\NC limit_below_vgap    \NC --                    \NC LowerLimitGapMin                         \NC big_op_spacing2 \NC\NR
\NC overbar_kern        \NC --                    \NC OverbarExtraAscender                     \NC default_rule_thickness \NC\NR
\NC overbar_rule        \NC --                    \NC OverbarRuleThickness                     \NC default_rule_thickness \NC\NR
\NC overbar_vgap        \NC --                    \NC OverbarVerticalGap                       \NC 3*default_rule_thickness \NC\NR
\NC quad                \NC --                    \NC <font_size(f)>                           \NC math_quad \NC\NR
\NC radical_kern        \NC --                    \NC RadicalExtraAscender                     \NC default_rule_thickness \NC\NR
\NC radical_rule        \NC --                    \NC RadicalRuleThickness                     \NC <not set> \NC\NR
\NC radical_vgap        \NC D, D'                 \NC RadicalDisplayStyleVerticalGap           \NC (default_rule_thickness+\crlf
                                                                                                    (abs(math_x_height)/4)) \NC\NR
\NC "                   \NC T, T', S, S', SS, SS' \NC RadicalVerticalGap                       \NC (default_rule_thickness+\crlf
                                                                                                    (abs(default_rule_thickness)/4)) \NC\NR
\NC space_after_script  \NC --                    \NC SpaceAfterScript                         \NC script_space \NC\NR
\NC stack_denom_down    \NC D, D'                 \NC StackBottomDisplayStyleShiftDown         \NC denom1 \NC\NR
\NC "                   \NC T, T', S, S', SS, SS' \NC StackBottomShiftDown                     \NC denom2 \NC\NR
\NC stack_num_up        \NC D, D'                 \NC StackTopDisplayStyleShiftUp              \NC num1 \NC\NR
\NC "                   \NC T, T', S, S', SS, SS' \NC StackTopShiftUp                          \NC num3 \NC\NR
\NC stack_vgap          \NC D, D'                 \NC StackDisplayStyleGapMin                  \NC 7*default_rule_thickness \NC\NR
\NC "                   \NC T, T', S, S', SS, SS' \NC StackGapMin                              \NC 3*default_rule_thickness \NC\NR
\NC sub_shift_down      \NC --                    \NC SubscriptShiftDown                       \NC sub1 \NC\NR
\NC sub_shift_drop      \NC --                    \NC SubscriptBaselineDropMin                 \NC sub_drop \NC\NR
\NC sub_sup_shift_down  \NC --                    \NC SubscriptShiftDown                       \NC sub2 \NC\NR
\NC sub_top_max         \NC --                    \NC SubscriptTopMax                          \NC (abs(math_x_height * 4) / 5) \NC\NR
\NC subsup_vgap         \NC --                    \NC SubSuperscriptGapMin                     \NC 4*default_rule_thickness \NC\NR
\NC sup_bottom_min      \NC --                    \NC SuperscriptBottomMin                     \NC (abs(math_x_height) / 4) \NC\NR
\NC sup_shift_drop      \NC --                    \NC SuperscriptBaselineDropMax               \NC sup_drop \NC\NR
\NC sup_shift_up        \NC D                     \NC SuperscriptShiftUp                       \NC sup1 \NC\NR
\NC "                   \NC T, S, SS,             \NC SuperscriptShiftUp                       \NC sup2 \NC\NR
\NC "                   \NC D', T', S', SS'       \NC SuperscriptShiftUpCramped                \NC sup3 \NC\NR
\NC sup_sub_bottom_max  \NC --                    \NC SuperscriptBottomMaxWithSubscript        \NC (abs(math_x_height * 4) / 5) \NC\NR
\NC underbar_kern       \NC --                    \NC UnderbarExtraDescender                   \NC 0 \NC\NR
\NC underbar_rule       \NC --                    \NC UnderbarRuleThickness                    \NC default_rule_thickness \NC\NR
\NC underbar_vgap       \NC --                    \NC UnderbarVerticalGap                      \NC 3*default_rule_thickness \NC\NR
\stoptabulate
...
\startsyntax
\directlua <16-bit number> <general text>!crlf
\directlua name <general text> <16-bit number> <general text>
\stopsyntax

The second \syntax{<general text>} is expanded fully, and then fed
into the \LUA\ interpreter state indicated by the \syntax{<16-bit
number>}. If the state does not exist yet, it will be initialized
...
\syntax{name <general text>} specifies the name of the \LUA\ chunk,
...
Expansion of macros etcetera in the \type{<general text>} is delayed
...
will be silently ignored. States are never closed automatically
except when a fatal out of memory error occurs, at which point
\LUATEX\ will exit anyway. Also be aware that \LUA\ states are not
closed immediately, but only when the \tex{output} routine comes
into play next (because there may be pending \tex{latelua} calls).
...
preloaded inside the executable, there are no external file dependancies.
\stopitemize
...
registers can be accessed and written to using five virtual
sub|-|tables of the
\luatex{tex} table:
...
\NC hold_head          \NC used for heldover items for next page\NC \NR
...
\TEX's \quote {current list}. There is no error checking yet!
...
with \tex{immediate\pdfximage}). The object number of the image
...
An \AFM\ file is a textual representation of (some of) the metainformation
...
Note: If you \type{fontforge.open()} a \TYPEONE\ file named \type{font.pfb},
...
\NC top_accent_horiz             \NC number   \NC only if set\NC\NR
...
\NC is_extender           \NC number     \NC \NC \NR
\NC startConnectorLength  \NC number     \NC \NC \NR
\NC endConnectorLength    \NC number     \NC \NC \NR
\NC fullAdvance           \NC number     \NC \NC \NR
\stoptabulate
...
limitation will be removed or perhaps it will become possibile to
...
virtual font. In case your font is refering to itself, you can use the
...
There is a fifth subnode type that is used excusively for delimiter 
...
First, there are the node types that are associated with the simple
math objects. All of these have the same list of fields.

\subsubsubsection{ord, bin, rel, open, close, punct, inner, vcenter,
  under, over nodes}

Valid fields: \showfields{ord}
...
\subsubsubsection{op nodes}

Valid fields: \showfields{op}

Operators are a bit special because they also have a sensible
\type{subtype}.

\starttabulate[|lT|l|p|]
\NC \ssbf field     \NC \bf type  \NC  \bf explanation \NC\NR
\NC subtype    \NC number         \NC  0 = \type{\displaylimits}, 
                                       1 = \type{\limits}, 
                                       2 = \type{\nolimits} \NC\NR
\NC attr       \NC \syntax{<node>}\NC          \NC\NR
\NC nucleus    \NC \syntax{<kernel>}\NC          \NC \NR
...
\item There is no synctex support yet (planned for 0.30)

\item LFS (\PDF\ Files larger than 2GiB) support is not working
properly yet.

...
\item An expection to the previous point: if the command
...
Returns the current process times cf.\ the \UNIX\ C library function
...
The new \type{epdf} library shall allow to scrutinize an external \PDF\ file.
...
The \type{epdf} library is in an early alpha state:
...
functions listed in the header files for the various classes,
...
as well as access to these source files.

There are many different userdata types defined
by the \type{epdf} library, currently these are
\type{Array}, \type{Catalog}, \type{Dict}, \type{GString},
\type{LinkDest}, \type{Object}, \type{ObjectStream}, \type{Page},
\type{PDFDoc}, \type{PDFRectangle}, \type{Ref}, \type{Stream},
\type{XRef}, and \type{XRefEntry} (more to follow).

All these class names and the Lua access functions closely resemble
the naming from the poppler header files,
...
\startfunctioncall
fontloader.apply_featurefile(<userdata> font, <string> filename)
\stopfunctioncall
...
\startfunctioncall
fontloader.apply_afmfile(<userdata> font, <string> filename)
\stopfunctioncall
...
Inside \tex{latelua} the object will marked for flushing.
...
will be zero cf. \tex{Udelcode} usage, but the returned table can have
...
sequential discretionaries as if they were a single object.
...
potential following ligature like \type{ic}. That means that \LUATEX\
can do away with one of the fields, and thus make the whole stuff fit
into just two discretionaries.

The mapping of the seven list fields to the six fields in this
discretionary pair is as follows:
...
subtype 5.  The reason for these special subtypes that not all of the
fields are in their \quote{normal} location, like the \type{<ff>-}
that appears in \type{disc2.replace}. Therefore it is necessary to be
able to distinguish this sequence of two joined discretionaries from
the case of a two standalone discretionaries appearing in a row.

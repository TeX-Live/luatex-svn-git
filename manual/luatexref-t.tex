...
In \LUATEX, it works like this: if \LUATEX\ senses an input character
that matches the value of the new integer parameter
\tex{exhyphenchar}, it will insert an automatic discretionary with the
current language's \tex{prehyphenchar} in the pre-break text (if it is
non-zero, otherwise the pre-break text will be empty), the current
language's \tex{posthyphenchar} in the post-break (also only if it is
non-zero), and the actual input character in the no-break text.

Initex sets the \type{\exhyphenchar=`\-}, and all languages start out
with \type{\prehyphenchar=`\-} and \type{\posthyphenchar=0}. When you
assign the values of \tex{prehyphenchar} and \tex{posthyphenchar}, you
are actually changing the settings for the current \tex{language},
this behaviour is compatible with \tex{patterns} and \tex{hyphenation}.
...
Work in this area is not yet complete: almost all of the possible
cases are handled by our rewritten ligaturing engine, except for the
fact that ligatures with the explicit hyphen character do not work
yet. We are working hard to make sure all of the possible inputs will
become supported soon.

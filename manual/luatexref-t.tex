...
yet, it will be initialized automatically. The current category codes
are applied to the \syntax{<general text>}, and it is passed on as if
it was displayed using \type{\the\toks}. On the \LUA\ side, each of
these blocks is treated as a chunk comprising  a single line. This
means that you can not use \LUA\ line comments (starting with
\type{--}) within the argument, as that will last for the rest of the 
input. You need to use \TEX-style comments (starting with \%) instead.
...
This primitive converts a \TEX\ token sequence so that it can be safely
...
consisting of a backslash with category code~12.
...
All \TEX\ fonts are represented to \LUA\ code as tables, and internally
...
Currently, only \TRUETYPE\ and \OPENTYPE\ fonts can be \quote{wide}
...
\OPENTYPE\ and \TRUETYPE\ fonts, every other font format essentially is treated as
\type{full}.
...
\item Seaching of directly-generated PDF documents that use wide fonts 
is impossible, due to explicit referencing (by \LUATEX) of font glyph
indices without a ToUnicode map being included.
...
mistake in the node list processing, \LUATEX\ may about with an
assertion error.
...
\item Use of Type1C for embedded PostScript font subsets.

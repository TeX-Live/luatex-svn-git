...
effects.  In particular, we are now using the built-in \type{io}
library, which means that some of the extended functionality is
currently not present (yet?), and that Lua's \type{tonumber()} may
return values in scientific notation, thereby confusing the \TEX\ end
of things. Some of the documentation below in this section is not
updated yet, while we are still deciding which extended features to
keep and which will be removed.

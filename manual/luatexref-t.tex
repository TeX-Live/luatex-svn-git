...
that are defined by the xpdf viewer library (written in C$+{}+$ by Derek Noonburg).
...
and many xpdf function bindings are still missing.
...
functions listed in the header files for the various xpdf classes,
e.\,g., files \type{PDFDoc.h}, \type{Dict.h}, and \type{Array.h}.
These files can be found in the xpdf subdirectory within the \LUATEX\ sources.
...
the naming from the xpdf header files,
...
(this is in|-|line with xpdf behavior).
...
\item \LUATEX's OTP (\tex{ocp} and \tex{ocplist}) processing is
deprecated, broken, and will be removed alltogether in the 0.70 beta.

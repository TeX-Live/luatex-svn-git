...
This callback is called when all document pages are already written to \PDF\
and \LUATEX\ is about to finalize the output document structure. Its intended
...
\type{fontloader.to_table()} on this object, returning a table
...
it will incrementally become possible to access the userdata object directly.
...
will be replaced by userdata objects (as much as needed to keep the
...
Note: starting with \LUATEX\ 0.63, \tex{hyphenation} exceptions are
case sensitive, and do not pay attention to \tex{uchyph} settings.
This means that you may have to add two exceptions in some cases.

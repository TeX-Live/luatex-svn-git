...
\startitemize
\item The major version is the integer result of \tex{luatexversion} divided by 100. The primitive is and \quote{internal variable}, so you may need to prefix it use with \type{\the} depending on the context.
\item The minor version is the two-digit result of \tex{luatexversion} modulo 100.
\item The revision is the given by \tex{luatexrevision}. This primitive expands to a positive integer.
...
\subsection{\tex{suppresslongerror}}
...
that is normally prohibitited (most prominently in the arguments
of non-long macros).

\subsection{\tex{suppressifcsnameerror}}
...
pairs, the \LUATEX\ process may hang indefinately.
...
tables representing tokens (see~\section[luatokens]). The third value
is always zero.
...
This function returns a list of the extra primitives that originate
...
\NC etex    \NC \ctxlua{for _,n in pairs(tex.extraprimitives('etex') do tex.print(n)) end } \NC \NR
\NC pdftex  \NC \ctxlua{for _,n in pairs(tex.extraprimitives('pdftex') do tex.print(n)) end } \NC \NR
\NC omega   \NC \ctxlua{for _,n in pairs(tex.extraprimitives('omega') do tex.print(n)) end } \NC \NR
\NC aleph   \NC \ctxlua{for _,n in pairs(tex.extraprimitives('aleph) do tex.print(n)) end } \NC \NR
\NC luatex  \NC \ctxlua{for _,n in pairs(tex.extraprimitives('luatex) do tex.print(n)) end } \NC \NR
\stoptabulate

Note that \type{luatex} does not contain \type{directlua}, as that is
considered to be a core primitive, along with all the \TEX82 primitives.

Running \type{tex.extraprimitives()} will give you the complete list,
it is equivalent to \type{tex.extraprimitives('etex', 'pdftex', 'omega', 'aleph', 'luatex')}
...
But if it is, then it will contruct a csname variable by concatenating the
...
Starting with version 0.40 (and this is why the above two functions
are needed), \LUATEX\ in \type{--ini} mode will contain only the \TEX82
...
\subsection{Scripts on horizontally extensible items like arrows}

The new primitives \tex{Uunderdelimiter} and \tex{Uoverdelimiter} allow the placement of
a subscript or superscript on an automatically extensible item. The vertical placements
are controlled by \tex{Umathunderdelimiterbgap}, \tex{Umathunderdelimitervgap},
...
The following items are all still todo before \LUATEX\ 0.40.

\startitemize
\item Pre-scripts.
\item Multi-story stacks.
\item Support for extensible big operators.
\item Make a \tex{mathstyle} parameter that can be queried during
  math list parsing (this implies a new primitive to deal with
  \type{{.. \over ..}} and \type{{.. \atop ..}}).
\item Staggered kerning (the \type{math_kern} array in \OPENTYPE).
\item Allow setting of the math parameters directly from Lua.
\item Implement \tex{Uoverunderdelimiters} (?).
...
\item The eight \quote{normal} noads will be merged into a single node id
   with subtypes.
\stopitemize
...
The values of \type{top_accent} and \type{bot_accent} are used only for math accent
placement, see the \at[math]{math chapter} in this manual for details.
...
\item There is no error given for \type{\outer} macros that are
expanded inside a group (This change may be temporary).

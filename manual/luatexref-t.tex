...
file name. The name will show up in double quotes with \type {string} prepended.
This can be avoided by starting the name with a \type {=}. Both the \type {@} and
\type {=} are part of the low level \LUA\ error reporting mechanism.
...
If you use the string version you can either assign a string or a function. When
at shipout time the node is consulted a string is translated to a function.
...
These numbers are in fact \ASCII\ values, so the 108 is for instance 
the numeric value of character \type {l}.

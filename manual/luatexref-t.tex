...
Starting from version 0.45, \LUATEX\ uses the kpathsea library to find
\type{require()}d modules, unless using kpathsea is explicitly disabled by the
user (this can be done by setting \type{texconfig.kpse_init} to
\type{false}). For this purpose, \type{package.loaders[2]} is replaced
by a different loader function that decides at runtime whether to use
kpathsea or the build-in core lua function.
...
\NC actual input (\POSTSCRIPT) name (this is the PS fontname in the incoming font source, new in 0.43)\NC\NR
\NC fullname           \NC no       \NC no       \NC yes \NC string \NC actual output (\POSTSCRIPT) name (this is the PS fontname that will be used in the PDF output)\NC\NR
...
\item \LUATEX~0.44.0 introduces two extra token lists, \tex{pdfxformresources} 

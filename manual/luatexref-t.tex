...
\starttabulate[|lT|p|]
\NC is_token     \NC checks if the given argument is a token userdatum \NC \NR
\NC get_next     \NC returns the next token in the input \NC \NR
\NC scan_keyword \NC returns true if the given keyword is gobbled \NC \NR
\NC scan_int     \NC returns a number \NC \NR
\NC scan_dimen   \NC returns a number representing a dimension \NC \NR
\NC scan_glue    \NC returns a glue spec node \NC \NR
\NC scan_toks    \NC returns a table of tokens token list (this can become a linked list in later releases) \NC \NR
\NC scan_code    \NC returns a character if its category is in the given bitset (representing catcodes) \NC \NR
\NC scan_string  \NC returns a string given between \type {{}} or as \type {\macro} \NC \NR
\NC create       \NC returns a userdata token object of the given control sequence name (or character); this interface can change  \NC \NR
\stoptabulate
...
(alkthough we can probably speed up the process in later versions if needed).

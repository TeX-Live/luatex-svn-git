...
vf_packet_bytes (charinfo *co) {
  ...
  while ((cmd = vf_packets[cur_packet_byte]) != packet_end_code) {
    ...
    case packet_nop_code:
      break;
    default: 
      pdf_error(maketexstring("vf"), maketexstring("invalid DVI command (1)"));     
    }
  };
  ...
  while ((cmd = vf_packets[cur_packet_byte]) != packet_end_code) {
    ...
    */
    ...
    case packet_nop_code:
      break;
    default: 
      pdf_error(maketexstring("vf"), maketexstring("invalid DVI command (2)"));     
    }
  };
  cur_h = save_cur_h;
  cur_v = save_cur_v;
...
integer *packet_local_fonts(internal_font_number f, integer *num) {
  ...
  for (c = font_bc(f);c <= font_ec(f);c++) {
	...
	case packet_image_code:
	  ...
	  break;
	default: 
	  pdf_error(maketexstring("vf"), maketexstring("invalid DVI command (3)"));     
	}
      }
    }
  }  
...
replace_packet_fonts(internal_font_number f, integer *old_fontid, integer *new_fontid, int count) {
  ...
  for (c = font_bc(f);c <= font_ec(f);c++) {
	...
	case packet_image_code:
	  ...
	  break;
	default: 
	  pdf_error(maketexstring("vf"), maketexstring("invalid DVI command (4)"));     
	}
      }
    }
  }  

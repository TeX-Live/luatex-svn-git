...
typedef enum
{ P_NONE, P_PAGE, P_TEXT, P_CHARARRAY, P_CHAR } pos_mode;
typedef enum
...
static void
calc_k1 (pdfstructure * p)
{
  p->k1 = exp10[p->pdf.h.e] / one_bp;
}

static void
calc_k2 (pdfstructure * p)
{
  p->tm[0].m =
...
static boolean
calc_pdfpos (pdfstructure * p, scaledpos * pos)
{
  scaledpos new;
...
static void
set_pos (pdfstructure * p, scaledpos * pos)
{
  boolean move;
...
static void
set_pos_temp (pdfstructure * p, scaledpos * pos)
{
  boolean move;
  assert (is_pagemode (p));
  move = calc_pdfpos (p, pos);
...
void
pdf_set_pos_temp (scaled h, scaled v)
{
  scaledpos pos;
  pos.h = h;
  pos.v = v;
...
static long
pdf_char_width (pdfstructure * p, internal_font_number f, int i)
{
...
void
pdf_print_charwidth (internal_font_number f, int i)
{
  pdffloat cw;
...
static void
begin_chararray (pdfstructure * p)
{
  assert (is_textmode (p));
  p->pdf_tj_pos = p->pdf;
  p->cw.m = 0;
...
static void
place_rule (pdfstructure * p, scaledpos * pos, scaled wd, scaled ht)
{
  pdfpos dim;
...
void
pdf_place_rule (scaled h, scaled v, scaled wd, scaled ht)
{
  scaledpos pos;
  pos.h = h;
  pos.v = v;
...
static void
setup_fontparameters (pdfstructure * p, internal_font_number f)
{
  p->f_cur = f;
...
static void
set_textmatrix (pdfstructure * p, scaledpos * pos)
{
  boolean move;
  assert (is_textmode (p));
  move = calc_pdfpos (p, pos);
...
void
pdf_place_glyph (internal_font_number f, integer c)
{
  scaledpos tmppos;
  tmppos.h = pos.h;
  tmppos.v = pos.v;
...
static void
place_form (pdfstructure * p, scaledpos * pos, integer i)
{
  goto_pagemode (p);
  pdf_printf ("q\n");
  set_pos_temp (p, pos);
...
void
pdf_place_form (scaled h, scaled v, integer i)
{
  scaledpos tmppos;
  tmppos.h = h;
  tmppos.v = v;

...
static const char *mplib_filetype_names[] =
    { "term", "error", "mp", "log", "ps", "mem", "tfm", "map", "pfb", "enc",
NULL };
...
static const char *special_fields[] = { "type", "prescript", NULL };

static const char *start_bounds_fields[] = { "type", "path", NULL };

static const char *start_clip_fields[] = { "type", "path", NULL };

static const char *stop_bounds_fields[] = { "type", NULL };

static const char *stop_clip_fields[] = { "type", NULL };

static const char *no_fields[] = { NULL };
...
static int mplib_get_char(void *f, mplib_instance * mplib_data)
{
    int c;
...
static void mplib_unget_char(void *f, mplib_instance * mplib_data, int c)
{
...
void mplib_close_file(MP mp, void *ff)
{
    if (ff != NULL) {
        mplib_instance *mplib_data = mplib_get_data(mp);
        void *f = ((File *) ff)->f;
        if (f != NULL && f != mplib_data->term_file_ptr
            && f != mplib_data->err_file_ptr && f != mplib_data->log_file_ptr
            && f != mplib_data->ps_file_ptr) {
            fclose(f);
        }
        free(ff);
    }
...
static void mplib_setup_file_ops(struct MP_options *options)
{
...
static int mplib_wrapresults(lua_State * L, mplib_instance * mplib_data, int h)
{
    lua_checkstack(L, 5);
    lua_newtable(L);
...
static int mplib_execute(lua_State * L)
{
    ...
    if (*mp_ptr != NULL && lua_isstring(L, 2)) {
        ...
        mplib_data->input_data =
            (char *) lua_tolstring(L, 2, &(mplib_data->input_data_len));
...
static void mplib_fill_field(lua_State * L, struct mp_fill_object *h)
{
    if (FIELD(path)) {
...
static void mplib_stroked_field(lua_State * L, struct mp_stroked_object *h)
{
    if (FIELD(path)) {
        mplib_push_path(L, h->path_p_field, MPLIB_PATH);
    } else if (FIELD(pen)) {
...
static void mplib_text_field(lua_State * L, struct mp_text_object *h)
{
    if (FIELD(text)) {
...
static void mplib_special_field(lua_State * L, struct mp_special_object *h)
{
    if (FIELD(prescript)) {
        lua_pushstring(L, h->pre_script_field);
    } else {
...
static void mplib_start_bounds_field(lua_State * L, struct mp_bounds_object *h)
{
    if (FIELD(path)) {
        mplib_push_path(L, h->path_p_field, MPLIB_PATH);
    } else {
...
static void mplib_start_clip_field(lua_State * L, struct mp_clip_object *h)
{
    if (FIELD(path)) {
        mplib_push_path(L, h->path_p_field, MPLIB_PATH);
    } else {
...
static int mplib_gr_index(lua_State * L)
{
    ...
    if (*hh) {
        ...
        if (mplib_is_S(type, 2)) {
            ...
            case mp_fill_code:
                mplib_fill_field(L, (mp_fill_object *) h);
                break;
            case mp_stroked_code:
                mplib_stroked_field(L, (mp_stroked_object *) h);
                break;
            case mp_text_code:
                mplib_text_field(L, (mp_text_object *) h);
                break;
            case mp_special_code:
                mplib_special_field(L, (mp_special_object *) h);
                break;
            case mp_start_clip_code:
                mplib_start_clip_field(L, (mp_clip_object *) h);
                break;
            case mp_start_bounds_code:
                mplib_start_bounds_field(L, (mp_bounds_object *) h);
                break;
                /* case mp_stop_clip_code:    */
                /* case mp_stop_bounds_code:  */
            default:

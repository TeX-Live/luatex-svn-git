...
pointer new_style(small_number s) { /* create a style node */
...
void show_math_node(pointer p) {
  ...
  case choice_node: 
    ...
    break;
  case ord_noad: case op_noad: case bin_noad: case rel_noad: case open_noad:
  case close_noad: case punct_noad: case inner_noad:
  case radical_noad: case over_noad: case under_noad: case vcenter_noad:
  case accent_noad: case left_noad: case right_noad:
...
void print_subsidiary_data(pointer p, ASCII_code c) {  /* display a noad field */
  if (cur_length>=depth_threshold) {
    ...
    case math_char: 
      print_ln(); 
      print_current_string(); print_fam_and_char(p);
      break;
    case sub_box: 
...
void print_style(integer c) {
  switch (c / 2) {
  case 0: tprint_esc("displaystyle"); break;
  case 1: tprint_esc("textstyle"); break;
  case 2: tprint_esc("scriptstyle"); break;
  case 3: tprint_esc("scriptscriptstyle"); break;
  default: tprint("Unknown style!");
  }
}
...
void display_normal_noad (pointer p) {
  switch (type(p)) {
  case ord_noad: tprint_esc("mathord"); break;
  case op_noad: tprint_esc("mathop"); break;
  case bin_noad: tprint_esc("mathbin"); break;
  case rel_noad: tprint_esc("mathrel"); break;
  case open_noad: tprint_esc("mathopen"); break;
  case close_noad: tprint_esc("mathclose"); break;
  case punct_noad: tprint_esc("mathpunct"); break;
  case inner_noad: tprint_esc("mathinner"); break;
  case over_noad: tprint_esc("overline"); break;
  case under_noad: tprint_esc("underline"); break;
  case vcenter_noad: tprint_esc("vcenter"); break;
  case radical_noad: 
    /*TH: TODO print oradical where needed and fix otherwise */
    tprint_esc("radical"); print_delimiter(left_delimiter(p));
    break;
  case accent_noad: 
    tprint_esc("accent"); print_fam_and_char(accent_chr(p));
    break;
  case left_noad: 
    tprint_esc("left"); print_delimiter(delimiter(p));
    break;
  case right_noad: 
    if (subtype(p)==normal)  tprint_esc("right");
    else tprint_esc("middle");
    print_delimiter(delimiter(p));
    break;
  }
  if (type(p)<left_noad) {
    if (subtype(p)!=normal) {
      if (subtype(p)==limits) tprint_esc("limits");
      else tprint_esc("nolimits");
    }
...
void display_fraction_noad(pointer p) {
  ...
  if (thickness(p)==default_code) tprint("= default");
...
void print_math_comp (halfword chr_code) {
  ...
  case inner_noad: tprint_esc("mathinner"); break;
  case under_noad: tprint_esc("underline"); break;
  default: tprint_esc("overline"); break;
  }
}

void print_limit_switch (halfword chr_code) {
  if (chr_code==limits) tprint_esc("limits");
  else if (chr_code==no_limits) tprint_esc("nolimits");
  else tprint_esc("displaylimits");
}
...
void scan_math(pointer p) {
 ...
 RESWITCH:
  ...
  case math_char_num_cmd: 
    if (cur_chr==0) scan_fifteen_bit_int();
    else scan_big_fifteen_bit_int();
    c=cur_val;
    break;
  case math_given_cmd: 
    c = ((cur_chr / 0x1000) * 0x1000000) +
         (((cur_chr % 0x1000) / 0x100) * 0x10000) +
         (cur_chr % 0x100);
    break;
  case omath_given_cmd: 
    ...
    break;
  case delim_num_cmd: 
    if (cur_chr==0)  scan_twenty_seven_bit_int();
...
void build_choices (void) {
  ...
  case 3: script_script_mlist(tail)=p; decr(save_ptr); return; break;
...
void finish_displayed_math (boolean l,
                            boolean danger,
                            pointer p,
                            pointer a ) ;
...
void finish_displayed_math (boolean l,
  ...
  */
  ...
  if (t!=adjust_head)  { /* migrating material comes after equation number */
    vlink(tail)=vlink(adjust_head); tail=t;
  }
  if (pre_t!=pre_adjust_head) {
    vlink(tail)=vlink(pre_adjust_head); tail=pre_t;
  }

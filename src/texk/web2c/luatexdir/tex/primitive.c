...
pointer
prim_lookup(str_number s)  /* search the primitives table */
{
    integer h; /* hash code */
...
boolean 
is_primitive(str_number csname)
{
    integer n,m;
    ...
    return ((n!=undefined_cs_cmd) &&
            ...
            (eq_type(n)==prim_eq_type(m)) &&
            (equiv(n)==prim_equiv(m)));
}
...
quarterword 
get_prim_eq_type(integer p)
{
    return prim_eq_type(p);
}

halfword 
get_prim_equiv(integer p)
{
    return prim_equiv(p);
}

str_number
get_prim_text(integer p)
{
    return prim_text(p);
}
...
pointer  
id_lookup(integer j, integer l) /* search the hash table */
{
    integer h; /* hash code */
    pointer p; /* index in |hash| array */
...
pointer
string_lookup (str_number s) /* search the hash table */
{
    integer h; /* hash code */
    pointer p; /* index in |hash| array */
    pool_pointer l;
    if (s<string_offset) {
        ...
        pool_pointer j;
        j=str_start_macro(s);
        if (s == str_ptr)  l = cur_length; else l = length(s);

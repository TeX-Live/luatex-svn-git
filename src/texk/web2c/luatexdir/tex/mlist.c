...
pointer fraction_rule(scaled t) {
  ...
  p=new_rule(); rule_dir(p)=math_direction;
  height(p)=t; depth(p)=0;
  return p;
}
...
pointer overbar(pointer b, scaled k, scaled t) {
  ...
  p=new_kern(k); vlink(p)=b; q=fraction_rule(t); vlink(q)=p;
...
void stack_into_box(pointer b, internal_font_number f, quarterword c) {
  ...
  p=char_box(f,c); vlink(p)=list_ptr(b); list_ptr(b)=p;
  height(b)=height(p);
}
...
pointer var_delimiter(pointer d, integer s, scaled v) {
  ...
  f=null_font; c=0; w=0; large_attempt=false;
    ...
    */
    if (char_tag(f,c)==ext_tag) {
      ...
      */
      ...
      cc=ext_bot(f,c);
      if (cc!=min_quarterword) w=w+height_plus_depth(f,cc);
      cc=ext_mid(f,c);
      if (cc!=min_quarterword) w=w+height_plus_depth(f,cc);
      cc=ext_top(f,c);
      if (cc!=min_quarterword) w=w+height_plus_depth(f,cc);
      ...
      if (u>0) {
        while (w<v) {
          w=w+u; incr(n);
          if ((ext_mid(f,c))!=min_quarterword) w=w+u;
        }
      }
      cc=ext_bot(f,c);
...
void flush_math (void) {
  flush_node_list(vlink(head)); flush_node_list(incompleat_noad);
...
pointer clean_box(pointer p, integer s) {
  ...
  cur_style=s; mlist_penalties=false;
...
void fetch(pointer a) { /* unpack the |math_char| field |a| */
  cur_c=mcharacter(a); 
  ...
  if (cur_f==null_font) {
    ...
    selector = new_string;
    print_size(cur_size); print_char(' ');
    print_int(fam(a)); tprint(" is undefined (character ");
    print_char(cur_c); print_char(')');
    s = make_string();
...
void make_under(pointer q) {
  ...
  pack_direction=math_direction; y=vpackage(x,0,additional,max_dimen);
...
void make_radical(pointer q) {
  ...
  if (cur_style<text_style) { /* display style */ 
    ...
    clr=default_rule_thickness; clr=clr + (abs(clr) / 4);
  }
  ...
  p=hpack(y,0,additional); vinfo(nucleus(q))=p; 
  math_type(nucleus(q))=sub_box;
}
...
void make_math_accent(pointer q) {
  ...
  if (char_exists(cur_f,cur_c)) {
    ...
    while (1) {
      if (char_tag(f,c)!=list_tag) break;
      y=char_remainder(f,c);
      if (!char_exists(f,y)) break;
      if (char_width(f,y)>w) break;
      c=y;
    }
    if (h<x_height(f)) delta=h; 
    else delta=x_height(f);
    ...
    pack_direction=math_direction;
    y=vpackage(y,0,additional,max_dimen); width(y)=width(x);
    if (height(y)<h) {
      /* Make the height of box |y| equal to |h| */
      p=new_kern(h-height(y)); vlink(p)=list_ptr(y); list_ptr(y)=p;
      height(y)=h;
    }
    vinfo(nucleus(q))=y;
    math_type(nucleus(q))=sub_box;
...
scaled make_op (pointer q) {
  ...
  if (math_type(nucleus(q))==math_char) {
    ...
    if ((cur_style<text_style)&&(char_tag(cur_f,cur_c)==list_tag)) { /* make it larger */
      ...
      if (char_exists(cur_f,c)) {
        cur_c=c; mcharacter(nucleus(q))=c;
      }
    }
  ...
  if (subtype(q)==limits) {
    ...
    if (math_type(supscr(q))==empty) {
       list_ptr(x)=null; flush_node(x); list_ptr(v)=y;
    } else  {
...
small_number make_left_right(pointer q, integer style, scaled max_d, scaled max_hv) {
  scaled delta, delta1, delta2; /* dimensions used in the calculation */
  pointer tmp;
  cur_style=style; setup_cur_size_and_mu();
...
void mlist_to_hlist (void) {
  ...
  RESWITCH: 
    ...
    case bin_noad: 
      switch (r_type) {
      case bin_noad: case op_noad: case rel_noad: case open_noad:
      case punct_noad: case left_noad:
    ...
    case ord_noad: 
      make_ord(q);
      break;
    case open_noad: case inner_noad: 
      break;
    case radical_noad: 
      make_radical(q);
      break;
    ...
    case choice_node: 
      ...
      case 3: choose_mlist(script_script_mlist); break; /* |script_script_style=6| */
    ...
    case whatsit_node: case penalty_node: case disc_node: 
      goto DONE_WITH_NODE;
      break;
    case rule_node:  
      if (height(q)>max_hl) max_hl=height(q);
      ...
      */
      if (subtype(q)==mu_glue) {
        ...
        if (p!=null) if ((type(p)==glue_node)||(type(p)==kern_node)) {
          vlink(q)=vlink(p); vlink(p)=null; flush_node_list(p);
        }
      }
      goto DONE_WITH_NODE;
      break;
    ...
    */
    ...
    case sub_box: 
      ...
      break;
    case sub_mlist:  
      cur_mlist=vinfo(nucleus(q)); save_style=cur_style;
      mlist_penalties=false; mlist_to_hlist(); /* recursive call */
      cur_style=save_style; setup_cur_size_and_mu();
  ...
  CHECK_DIMENSIONS: 
      ...
      if (height(z)>max_hl) max_hl=height(z);
      if (depth(z)>max_d) max_d=depth(z);
      list_ptr(z)=null; flush_node(z);
  DONE_WITH_NOAD: 
      r=q; r_type=type(r);
      if (r_type==right_noad) {
        r_type=left_noad; cur_style=style; setup_cur_size_and_mu();
      }
  DONE_WITH_NODE: 
      q=vlink(q);
  }
  ...
  */
  ...
  while (q!=null) {
    ...
    switch (type(q)) {
    case op_noad: case open_noad: case close_noad: case punct_noad: case inner_noad: 
      t=type(q);
      break;
    case bin_noad: 
      t=bin_noad; pen=bin_op_penalty;
      break;
    case rel_noad:
      t=rel_noad; pen=rel_penalty;
      break;
    case ord_noad: case vcenter_noad: case over_noad: case under_noad: 
      break;
    case radical_noad: 
      s=radical_noad_size;
      break;
    case accent_noad: 
      ...
      break;
    case fraction_noad: 
      t=inner_noad; s=fraction_noad_size;
      break;
    ...
    case style_node: 
      ...
      goto DELETE_Q;
      break;
    case whatsit_node: case penalty_node: case rule_node: case disc_node:
    case adjust_node: case ins_node: case mark_node: case glue_node: case kern_node:
      vlink(p)=q; p=q; q=vlink(q); vlink(p)=null; 
      goto DONE;
      break;
    default:
    ...
    if (r_type>0) { /* not the first noad */
      ...
      case '3': 
        if (cur_style<script_style)
          x=param_med_mu_skip_code;
        else 
          x=0;
        break;
      case '4': 
        if (cur_style<script_style) 
          x=param_thick_mu_skip_code;
        else 
          x=0;
        break;
      default: 
    ...
    if (new_hlist(q)!=null) {
      vlink(p)=new_hlist(q);
      do { p=vlink(p); } while (vlink(p)!=null);
    }
    if (penalties) if (vlink(q)!=null) if (pen<inf_penalty) {
       r_type=type(vlink(q));
       if (r_type!=penalty_node)  if (r_type!=rel_noad) {
         z=new_penalty(pen); vlink(p)=z; p=z;
       }
    }
    if (type(q)==right_noad)
      t=open_noad;

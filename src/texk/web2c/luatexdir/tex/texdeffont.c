...
char *
scaled_to_string(scaled s)  /* prints scaled real, rounded to five digits */
{ 
...
void tex_def_font (small_number a)
{
    ...
    get_r_token(); 
    u=cur_cs;
    if (u>=null_cs) t=text(u); else t=maketexstring("FONT");
    if (a>=4) {
    ...
    if (scan_keyword("at")) {
        ...
        if ((s<=0)||(s>=01000000000)) {
            char err[256];
            char *errhelp[] = {"I can only handle fonts at positive sizes that are", 
        ...
        if ((cur_val<=0)||(cur_val>32768)) {
            char err [256];
            char *errhelp[] = {"The magnification ratio must be between 1 and 32768.", NULL};
    ...
    if (scan_keyword("offset")) {
        ...
        if (cur_val<0) {
            ...
            snprintf(err,255,"Illegal offset has been changed to 0 (%d)", (int)cur_val);
            tex_error(err, errhelp); 
            offset=0;
        }
    }

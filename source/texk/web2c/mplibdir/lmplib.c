...
typedef enum {
    ...
    P_INTERACTION, P_INI_VERSION, P_MEM_NAME, P_JOB_NAME, P_FIND_FILE, P_RUN_SCRIPT,
...
static char *mplib_run_script(MP mp, const char *str)
{
    ...
    /* tracer */
    lua_getfield(L, LUA_REGISTRYINDEX, "mplib_run_script");
    /* tracer function */
    lua_pushstring(L,str);
    /* tracer function string */
    i = lua_pcall(L, 1, 1,-2);
    /*    lua_remove(L, base); */    /* remove traceback function */
    if (i != 0) {
        lua_gc(L, LUA_GCCOLLECT, 0);
        /* for the moment assume error handling elsewhere */
        // L = luatex_error(L, (i == LUA_ERRRUN ? 0 : 1));
        return "";
    } else {
        ss = lua_tolstring(L, -1, &ll);
        s = xmalloc(ll+1);
        memcpy(s,ss,ll+1);
        lua_pop(L, 1);
        return s;
...
static int mplib_run_script_function(lua_State * L)
{
    ...
    lua_pushstring(L, "mplib_run_script");
    lua_pushvalue(L, -2);
    lua_rawset(L, LUA_REGISTRYINDEX);
    return 0;
}
            ...
            for (i = 0; mplib_parms[i].name != NULL; i++) {
                ...
                case P_RUN_SCRIPT:
                    if (mplib_run_script_function(L)) {  /* error here */
                        fprintf(stdout,
                                "Invalid arguments to mp.new({run_script=...})\n");
                    }
                    break;
                default:
                    break;

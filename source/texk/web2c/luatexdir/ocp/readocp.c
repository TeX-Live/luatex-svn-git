...
read_ocp_info(pointer u, str_number nom, str_number aire, str_number ext,
  ...
  g=null_ocp; f=null_ocp; cnam=NULL;
  if (external_ocp) {
    ...
    if (ocp_buffer!=NULL) 
      xfree(ocp_buffer);
    ocp_cur=0; ocp_buffer=NULL; ocp_size=0;
    callback_id=callback_defined(find_ocp_file_callback);
    if (callback_id>0) {
      res = run_callback(callback_id,"S->S",(char *)(nameoffile+1), cnam);
    ...
    if (callback_id>0) {
      ...
      if (!res) ocp_abort("callback error");
      if (!file_opened) ocp_abort("opening file");
    } else {
      FILE *ocp_file = NULL;
      if (!ocp_open_in(ocp_file)) ocp_abort("opening file");
      file_opened=true;
      res = read_ocp_file(ocp_file,&ocp_buffer,&ocp_size);
      b_close(ocp_file);
      if (!res) ocp_abort("reading file");
    }
    if (ocp_size==0) ocp_abort("checking size");
...
BAD_OCP: 
  {
    ...
    if (file_opened) {
      snprintf(errmsg, 255, "Translation process \\%s=%s not loadable: Bad ocp file", c, cnam);
    } else {
      snprintf(errmsg, 255, "Translation process \\%s=%s not loadable: ocp file not found", c, cnam);
    }

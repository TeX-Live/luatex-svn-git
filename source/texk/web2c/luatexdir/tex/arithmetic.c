...
scaled round_decimals(int k)   /* converts a decimal fraction */
{
...
void print_scaled(scaled s) /* prints scaled real, rounded to five digits */
{
...
static integer ab_vs_cd(integer a, integer b, integer c, integer d)
{
	...
	if (q==0) 
	    return -1;
	a=b; b=q; c=d; d=r;  /* now |a>d>0| and |c>b>0| */
    }
}

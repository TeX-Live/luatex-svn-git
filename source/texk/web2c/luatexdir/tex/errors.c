...
#define new_line_char int_par(param_new_line_char_code)
...
void error (void) /* completes the job of error reporting */
{
	...
	if (callback_id>0) t = run_callback(callback_id,"->");
	show_context();
	if (haltonerrorp) {
	...
	if (interaction==error_stop_mode) {
		...
		CONTINUE:
			...
			case 'Q': case 'R': case 'S':
				...
				tprint("..."); print_ln(); update_terminal(); 
				return;
				break;
			case 'X':
			...
			if (base_ptr>0) tprint("E to edit your file,");
			if (deletions_allowed) 
...
{
	tprint(" ("); print_int(n); print_char(')'); error();
}
...
void lua_norm_error(char *s) /* lua found a problem */
{
	integer saved_new_line_char;
	saved_new_line_char=new_line_char;
	new_line_char=10;
	print_err("LuaTeX error "); tprint(s);
...
void lua_fatal_error(char *s) /* lua found a problem */
{
	new_line_char=10; normalize_selector();
	print_err("LuaTeX fatal error "); tprint(s);
	succumb();
}

/* Here is the most dreaded error message */

void overflow(char *s, integer n) /* stop due to finiteness */
{
	normalize_selector();
	print_err("TeX capacity exceeded, sorry [");
	tprint(s); print_char('='); print_int(n); print_char(']');
...
void confusion(char *s) /* consistency check violated; |s| tells where */
{
	normalize_selector();
	if (history<error_message_issued) {
		print_err("This can't happen ("); tprint(s); print_char(')');

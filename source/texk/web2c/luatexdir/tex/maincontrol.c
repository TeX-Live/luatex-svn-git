...
void main_control (void) /* governs \TeX's activities */
{
 ...
 RESWITCH: 
	...
	CONTINUE: 
		...
		if ((chr_stack==ex_hyphen_char)&&(cur_chr!=ex_hyphen_char)&&(mode>0)) {
	...
	if ((abs(mode)+cur_cmd==hmode+spacer_cmd)&& (!(space_factor == 1000))) {
	    app_space();
	} else {
	...
	t=cur_chr; 
	get_x_token();
	if ((cur_cmd==letter_cmd)||(cur_cmd==other_char_cmd)||(cur_cmd==char_given_cmd)||
	    (cur_cmd==char_num_cmd)) {
...
boolean its_all_over (void) /* do this when \.{\\end} or \.{\\dump} occurs */
{
    if (privileged()) {
...
void append_glue (void)
{
    ...
    case skip_code: scan_glue(glue_val_level); break;
    case mskip_code: scan_glue(mu_val_level); break;
...
void extra_right_brace (void) 
{
    ...
    switch (cur_group) {
    case semi_simple_group: tprint_esc("endgroup"); break;
    case math_shift_group: print_char('$'); break;
    case math_left_group: tprint_esc("right"); break;
    }
...
void scan_box(integer box_context)
{
    ...
    } else if ((box_context>=leader_flag)&&((cur_cmd==hrule_cmd)||(cur_cmd==vrule_cmd))) {
...
void prefixed_command (void)
{
	...
	p=cur_cs;
	if (n==normal) {
	    do { get_token(); } while (cur_cmd==spacer_cmd);
	...
	*/
	...
	default: 
	    scan_register_num();
	    switch(n) {
	    case count_def_code: define(p,assign_int_cmd,count_base+cur_val); break;
	    case attribute_def_code: define(p,assign_attr_cmd,attribute_base+cur_val); break;
	    case dimen_def_code: define(p,assign_dimen_cmd,scaled_base+cur_val); break;
	    case skip_def_code: define(p,assign_glue_cmd,skip_base+cur_val); break;
	    case mu_skip_def_code: define(p,assign_mu_glue_cmd,mu_skip_base+cur_val); break;
	    case toks_def_code: define(p,assign_toks_cmd,toks_base+cur_val); break;
	    default: confusion("shorthand_def"); break;
	    }
	    break;
	}
	break;
	...
	*/ 
	switch (cur_chr) {
	case 0: new_hyph_exceptions(); break;
	case 1: new_patterns(); break;
	case 2: new_pre_hyphen_char(); break;
	case 3: new_post_hyphen_char(); break;
	case 4: new_pre_exhyphen_char(); break;
	case 5: new_post_exhyphen_char(); break;
	}
	break;
	...
	if (n == no_lig_code) {
	    ...
	    scan_int();
	    switch (n) {
	    case lp_code_base: set_lp_code(f, p, cur_val); break;
	    case rp_code_base: set_rp_code(f, p, cur_val); break;
	    case ef_code_base: set_ef_code(f, p, cur_val); break;
	    case tag_code:     set_tag_code(f, p, cur_val); break;
	    }
	}
	break;
    case def_font_cmd:
...
{
 RESTART: 
    do { get_token(); } while (cur_tok==space_token);
...
{  
	...
	} else if ((p==(int_base+output_box_code)&&((cur_val>65535)|(cur_val<0)))) {
...
void do_register_command(int a)
{
    ...
    p=cur_chr; 
    scan_register_num();
    if (p==int_val_level) l=cur_val+count_base;
    else if (p==attr_val_level) l=cur_val+attribute_base;
    else if (p==dimen_val_level) l=cur_val+scaled_base;
    else if (p==glue_val_level) l=cur_val+skip_base;
    else if (p==mu_val_level) l=cur_val+mu_skip_base;
...
void show_whatever (void)
{
	...
	if (cond_ptr==null) {
	    ...
	    p=cond_ptr; 
	    n=0;
	    do { incr(n); p=vlink(p); } while (p!=null);
	    p=cond_ptr; 
	    t=cur_if; 
	    l=if_line; 
	    m=if_limit;

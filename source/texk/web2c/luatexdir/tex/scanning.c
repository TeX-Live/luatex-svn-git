...
void scan_left_brace (void) /* reads a mandatory |left_brace| */
{
    ...
    do { get_x_token(); } while ((cur_cmd==spacer_cmd)||(cur_cmd==relax_cmd));
...
void scan_optional_equals (void)
{
    ...
    do { get_x_token(); } while (cur_cmd==spacer_cmd);
...
static boolean short_scan_something_internal (int cmd, int chr, int level, boolean negative)
{
	...
	else if (m==2)  cur_val=interaction; /* interactionmode */
	...
	if (m>=last_item_input_line_no_code) {
		...
		if (m<last_item_eTeX_mu) {
		    switch(m) {
		    case last_item_mu_to_glue_code: scan_mu_glue(); break;
		    }; /* there are no other cases */
		    cur_val_level=glue_val_level;
		} else if (m<last_item_eTeX_expr) {
		    switch (m) {
		    case last_item_glue_to_mu_code: scan_normal_glue(); break;
		    } /* there are no other cases */
		...
		*/
		...
		case last_item_font_char_ic_code: 
			...
			switch (m) {
			case last_item_font_char_wd_code: cur_val=char_width(q,cur_val); break;
			case last_item_font_char_ht_code: cur_val=char_height(q,cur_val); break;
			case last_item_font_char_dp_code: cur_val=char_depth(q,cur_val); break;
			case last_item_font_char_ic_code: cur_val=char_italic(q,cur_val); break;
			} /* there are no other cases */
		    } else {
			cur_val=0;
		    }
		...
		case last_item_par_shape_dimen_code: 
			...
			if (cur_val>vinfo(par_shape_ptr+1))  
			    cur_val=vinfo(par_shape_ptr+1);
			cur_val=varmem[par_shape_ptr+2*cur_val-q+1].cint;
		    }
		    cur_val_level=dimen_val_level;
		    break;
		...
		switch (m) {
		case last_item_input_line_no_code: cur_val=line; break;
		case last_item_badness_code: cur_val=last_badness;  break;
		case last_item_pdftex_version_code:  cur_val = get_pdftexversion();  break;
		case last_item_luatex_version_code:  cur_val = get_luatexversion(); break;
		case last_item_pdf_last_obj_code:    cur_val = pdf_last_obj; break;
		case last_item_pdf_last_xform_code:  cur_val = pdf_last_xform; break;
		case last_item_pdf_last_ximage_code: cur_val = pdf_last_ximage; break;
		case last_item_pdf_last_ximage_pages_code: cur_val = pdf_last_ximage_pages; break;
		case last_item_pdf_last_annot_code:  cur_val = pdf_last_annot; break;
		case last_item_pdf_last_x_pos_code:  cur_val = pdf_last_x_pos; break;
		case last_item_pdf_last_y_pos_code:  cur_val = pdf_last_y_pos; break;
		case last_item_pdf_retval_code:      cur_val = pdf_retval; break;
		case last_item_pdf_last_ximage_colordepth_code: cur_val = pdf_last_ximage_colordepth; break;
		case last_item_random_seed_code:  cur_val = random_seed; break;
		case last_item_pdf_last_link_code:   cur_val = pdf_last_link; break;
		case last_item_Aleph_version_code: cur_val = get_Aleph_version(); break;
		case last_item_Omega_version_code: cur_val = get_Omega_version(); break;
		case last_item_eTeX_version_code: cur_val = get_eTeX_version(); break;
		case last_item_Aleph_minor_version_code: cur_val = get_Aleph_minor_version(); break;
		case last_item_Omega_minor_version_code: cur_val = get_Omega_minor_version(); break;
		case last_item_eTeX_minor_version_code: cur_val = get_eTeX_minor_version(); break;
		case last_item_current_group_level_code: cur_val = cur_level-level_one; break;
		case last_item_current_group_type_code: cur_val = cur_group; break;
		case last_item_current_if_level_code: 
		    q=cond_ptr; cur_val=0;
		    while (q!=null) {
			incr(cur_val); q=vlink(q);
		    }
		    break;
		...
		cur_val_level=int_val_level;
		if ((cur_list.tail_field==cur_list.head_field)||(cur_list.mode_field==0)) 
		    cur_val=-1;
	    }  else {
...
void scan_something_internal(int level, boolean negative)
{
 ...
 RESTART: 
	...
	case def_del_code_cmd: 
		...
		cur_val1=get_cat_code(int_par(param_cat_code_table_code),cur_val);
		scanned_result(cur_val1,int_val_level);
	...
	case assign_font_int_cmd: 
		...
		k = cur_val;
		switch (m) {
		case lp_code_base: scanned_result(get_lp_code(n, k),int_val_level); break;
		case rp_code_base: scanned_result(get_rp_code(n, k),int_val_level); break;
		case ef_code_base: scanned_result(get_ef_code(n, k),int_val_level); break;
		case tag_code: scanned_result(get_tag_code(n, k),int_val_level); break;
		}
	    }
	    break;
	case register_cmd: 
	    ...
	    scan_register_num();
	    switch (m) {
	    case int_val_level: cur_val=count(cur_val); break;
	    case attr_val_level: cur_val=attribute(cur_val); break;
	    case dimen_val_level: cur_val=dimen(cur_val); break;
	    case glue_val_level: cur_val=skip(cur_val); break;
	    case mu_val_level: cur_val=mu_skip(cur_val); break;
	    } /* there are no other cases */
	    cur_val_level=m;
	    break;
...
void scan_int (void) /* sets |cur_val| to an integer */
{
...
/* sets |cur_val| to a dimension */
{
    ...
    */
	...
	if (scan_keyword("filll")) { cur_order=filll; goto ATTACH_FRACTION; }
	else if (scan_keyword("fill")) { cur_order=fill; goto ATTACH_FRACTION; }
	else if (scan_keyword("fil")) { cur_order=fil; goto ATTACH_FRACTION; }
	else if (scan_keyword("fi")) { cur_order=sfi; goto ATTACH_FRACTION; }
...
void  scan_glue(int level) /* sets |cur_val| to a glue spec pointer */
{
    boolean negative; /* should the answer be negated? */
...
void scan_scaled (void) /* sets |cur_val| to a scaled value */
{
    boolean negative; /* should the answer be negated? */
    integer f; /* numerator of a fraction whose denominator is $2^{16}$ */
    int k,kk; /* number of digits in a decimal fraction */
    halfword p,q; /* top of decimal digit stack */
    f=0; arith_error=false; negative=false;
    /* Get the next non-blank non-sign... */
    negative=false;
    do {
	/* Get the next non-blank non-call token */

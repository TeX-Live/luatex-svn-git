...
static int
do_lua_kpathsea_lookup (lua_State * L, kpathsea kpse, int idx)
{
  int i;
  string ret = NULL;
  ...
  if (lua_type(L,idx+1) == LUA_TTABLE) {
	  ...
	  while (lua_next(L, -2) != 0) {   /* numeric value */
	      if (lua_isstring(L, -1)) {
		  str_list_add (&subdir_paths, lua_tostring(L,-1));
	      }
	      lua_pop(L,1);
	  }
...
static int
lua_kpathsea_lookup (lua_State * L)
{
